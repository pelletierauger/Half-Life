...
let chromaticGlitch = new ShaderProgram("chromatic-glitch");

chromaticGlitch.vertText = undulatingGlitch.vertText;

chromaticGlitch.fragText = `
// beginGLSL
precision mediump float;
// Passed in from the vertex shader.
uniform float time;
...
void main() {
    vec2 uv = vec2(gl_FragCoord.xy) / vec2(1600, 1600);
    float rando = rand(vec2(uv.x, uv.y));
    float t = time + 270. - 10e-2;
            // vec2 uv = fragCoord.xy / iResolution.xy;
    // -----------------------------------------------------------------
    //     Chromatic aberration from there:
    ...
    amount *= 0.05;
    uv = v_texcoord;
        if (sin(uv.y * 50. * 0.5) + sin(uv.y * 10. * 0.5) + sin(uv.y * 10. * 0.5 * t) < -0.0) {
            // uv += (pow(cos(uv.y * uv.x * 1e18), 0.0125) + sin(uv.x * 1e-2 + tan(uv.y * t * 1.))) * 2. * floor(sin(uv.y * 1e-3 * ((floor(sin(t * 1e-8 * sin(t * 1e-2))))))) + sin(uv.y * 1e12 + uv.y * 1e-2 * sin(20. * 1e-10) + tan(t * 1e-10)) * 0.0625 * 1e-5;
            float dx = 40. * (1. / 2560. * 0.5);
            float dy = 25. * (1. / 1440. * 0.5);
            vec2 coord = vec2(dx * floor(uv.x / dx) * 0.65, dy * floor(uv.y / dy));
                            coord.y = coord.y + sin(coord.y * t * 1e1) * 0.1;
...
// endGLSL
`;
chromaticGlitch.init();


let lakeGlitch = new ShaderProgram("lake-glitch");

lakeGlitch.vertText = undulatingGlitch.vertText;
...
void main() {
    vec2 uv = vec2(gl_FragCoord.xy) / vec2(1600, 1600);
    float rando = rand(vec2(uv.x, uv.y));
    float t = time + 270. - 10e-10;
    float tt = floor(sin(t * 1e-2) * 10.);
    // if (sin(uv.y * 50. * 0.5) + sin(uv.y * 10. * 0.5) + sin(uv.y * 10. * 0.5 * tt) < 0.9) {
        // gl_FragColor = vec4(1., 1., 1., 1.);   
        vec2 vv = v_texcoord;
        // vv.y = vv.x + sin(vv.y * t * 1e3);
        // gl_FragColor = texture2D(u_texture, vv + (pow(cos(uv.y * uv.x * 1e18), 0.0125) + sin(uv.x * 1e-2 + tan(uv.y * t * 1.))) * 2. * floor(sin(uv.y * 1e-3 * ((floor(sin(t * 1e-8 * sin(t * 1e-2))))))) + sin(uv.y * 1e12 + uv.y * 1e-2 * sin(20. * 1e-10) + tan(t * 1e-10)) * 0.0625 * 1e-5) * 1.;
        float dx = 40. * (1. / 2560. * (sin(0.06251e5 * vv.x)));
        float dy = 25. * (1. / 1440. * (sin(0.06251e5 * vv.x)));
        vec2 coord = vec2(dx * floor(uv.x / dx) * 0.65, dy * floor(uv.y / dy));
        // tc = texture2D(sceneTex, coord).rgb;
                // coord.y = coord.y + sin(coord.y * t * 1e1) * 0.1;
        gl_FragColor = texture2D(u_texture, coord) - (rando * 0.125) * 1.;
