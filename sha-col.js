...
//     varying vec2 center;
    ...
    void main(void) {
        ...
        float rando = rand(pos);
        // gl_FragColor = vec4(1.0, (1.0 - dist_squared * 40.) * 0.6, 0.0, alpha + ((0.12 - dist_squared) * 4.) - (rando * 0.2));
        gl_FragColor = vec4(1.0, 0.4 - dist_squared, 0. + alpha * 120., ((3. - dist_squared * 24.0 * (0.25 + alph) - (rando * 1.1)) * 0.045 + alpha)) * 1.25;
//         gl_FragColor = gl_FragColor.gbga;
        gl_FragColor.rgb = vec3(1.0) - gl_FragColor.grb;
            // gl_FragColor.rgb = hueShift2(gl_FragColor.rgb, PI * -0.5);
        // gl_FragColor.rgb = ContrastSaturationBrightness(gl_FragColor.rgb, 1., 1., 1.);
             // gl_FragColor = gl_FragColor.grba;
        // gl_FragColor.r += 0.25;
        gl_FragColor.b *= 0.35;
        gl_FragColor.g += 0.6 * sin(ind * 1e-5);
...
// 
    void main(void) {
        ...
        float osc = map(sin(t * 16e-1), -1., 1., 0.05, 4.005);
        float i = vertexID * 1e-1;
        float x = cos(i * 1.5e-6 * i) * i * 24e-3;
        float y = sin(i * 1.5e-6 * i) * i * 24e-3;
        x += tan(tan(i * i * cos(i * 1e2 * sin(i * 2e3)) * 1e3) + osc * 2e1 / i * 1e4) * 5000.5;
        y += tan(tan(i * i * sin(i * 1e2 * sin(i * 2e3)) * 1e3) + osc * 2e1 / i * 1e4) * 5000.5;
//         x *= 0.25 * 44.5;
        // y += cos(x * i) * x * 2e1;
...
//     varying vec2 center;
    ...
    void main(void) {
        ...
        float rando = rand(pos);
        // gl_FragColor = vec4(1.0, (1.0 - dist_squared * 40.) * 0.6, 0.0, alpha + ((0.12 - dist_squared) * 4.) - (rando * 0.2));
        gl_FragColor = vec4(1.0, 0.4 - dist_squared, 0. + alpha * 120., ((3. - dist_squared * 24.0 * (0.25 + alph) - (rando * 1.5)) * 0.045 + alpha)) * 1.25;
//         gl_FragColor = gl_FragColor.gbga;
        gl_FragColor.rgb = vec3(1.0) - gl_FragColor.grb;
            // gl_FragColor.rgb = hueShift2(gl_FragColor.rgb, PI * -0.5);
        // gl_FragColor.rgb = ContrastSaturationBrightness(gl_FragColor.rgb, 1., 1., 1.);
        ...
        gl_FragColor.b *= 0.35;
        gl_FragColor.g += 0.6 * sin(ind * 1e-5);
        // gl_FragColor.r += 0.6 * sin(ind * 1e-5);
        // gl_FragColor.b += 0.2 * sin(ind * 1e-5);
        gl_FragColor.rgb *= 1.1;
                // gl_FragColor.rgb = gl_FragColor.grb;
        gl_FragColor.rgb = mix(gl_FragColor.rgb, gl_FragColor.rrr, 0.5);
        // gl_FragColor = gl_FragColor.rbga;
