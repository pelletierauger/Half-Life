...
// 
    void main(void) {
        ...
        x += sin(sin(x * 0.25 * sin(i * 3. + t * 2e1)) * i * 3e5) * 0.95;
        y += sin(cos(y * 0.25 * sin(i * 3. + t * 2e1)) * i * 3e5) * 0.95;
        // x += cos(cos(cos(i * 0.1 * tan(i * 1e-2))) + t * 10.) * 0.005;
        // y += sin(cos(sin(i * 0.1 * tan(i * 1e-2))) + t * 10.) * 0.005;
        // x += tan(pow(x, 1e-2 / i * t * 1e3) * i + t * 2e-5 * i) * 2e-6;
        // y += tan(pow(y, 1e-2 / i * t * 1e3) * i + t * 2e-5 * i) * 2e-6;
        // x += cos(x * sin(t * 2e1)) * 0.1;
        // y += sin(y * sin(t * 2e1)) * 0.1;
//         x *= 0.25 * 44.5;
//         y *= 0.25 * 44.5;
        // x += cos(t * 10.75) * i * 0.0000025;
        // y += sin(t * 10.75) * i * 0.0000025;
        // x += 1. * cos(t * 5.);
        // y += 1. * sin(t * 5.);
        x *= 1.5;
        y *= 1.5;
