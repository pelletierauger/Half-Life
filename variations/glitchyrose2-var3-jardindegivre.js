...
// 
    void main(void) {
        ...
        float x = cos(i * 1e2) * i * 1e-5 * 2.;
        float y = sin(i * 1e2) * i * 1e-5 * 2.;
        x += sin(x * 0.25 * sin(t * 1e1) * sin(i * 3. + x * 0.5e2) + sin(x * 1e-2)) * 0.75;
        y += sin(y * 0.25 * sin(t * 1e1) * sin(i * 3. + x * 0.5e2) + sin(x * 1e-2)) * 0.75;
        x += tan(cos(i * 69. * tan(i * 1e3)) * 1e6 * sin(1e-5) + t * 1e1) * 2.5;
        y += tan(sin(i * 69. * tan(i * 1e3)) * 1e6 * sin(1e-5) - t * 1e1) * 2.5;
        //         x *= 0.25 * 44.5;
//         y *= 0.25 * 44.5;
        // x += cos(t * 100.75 * i * 5e-6) * i * 0.000005;
        // y += sin(t * 100.75 * i * 5e-6) * i * 0.000005;
        // x += 1. * cos(t * 5.);
        // y += 1. * sin(t * 5.);
        x *= 1.5e-1 * 2.;
        y *= 1.5e-1 * 2.;
