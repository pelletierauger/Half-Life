...
// 
    void main(void) {
        ...
        x *= cos(x / sin(x * 1e2) * x + t * 1e2) * 0.65;
        y *= sin(y / sin(y * 1e2) * x + t * 1e2) * 0.65;
//         x *= 0.25 * 44.5;
//         y *= 0.25 * 44.5;
        // x += cos(t * 100.75) * i * 0.000005;
        // y += sin(t * 100.75) * i * 0.000005;
        // x += 1. * cos(t * 5.);
        // y += 1. * sin(t * 5.);
        x *= 2.5;
        y *= 2.5;
        ...
        // float xx = x + cos(y * 1. * sin((100000. + y) * 1e-1)) * 2.;
                // x = mix(x, xx, map(sin(t * 1. * sin(t * 2.)), -1., 1., 0., 1.));
        // float yy = y + cos(x * 10. * sin((100000. + x) * 1e-1)) * 2.;
        // x = mix(x, xx, 0.2);
        // y = mix(y, yy, 0.2);
        // y *= map(sin(t * 40.), -1., 1., 1., 2.5);
        vec2 v = vec2(x, y) * vec2(1.25, 1.25);
        // x += cos(t * 1e18) * 1.;
        // y += sin(t * 1e18) * 1.;
//         float x = cos(i) * i * 1e-5 * 2.;
//         float y = sin(i) * i * 1e-5 * 2.;
        gl_Position = vec4(v.x * 0.6 * 1., v.y * 1., 0.0, 1.0);
...
// 
    void main(void) {
        ...
        float x = cos(i + sin(i * 1e-4 + i * 1e-4)) * i * 1e-5;
        float y = sin(i + sin(i * 1e-4 + i * 1e-4)) * i * 1e-5;
        x *= cos(x / sin(x * 1e2 + x * 1e2) * x * t * 1e2) * 1.65;
        y *= sin(y / sin(y * 1e2 + x * 1e2) * x * t * 1e2) * 1.65;
//         x *= 0.25 * 44.5;
//         y *= 0.25 * 44.5;
        // x += cos(t * 100.75) * i * 5e-7;
        // y += sin(t * 100.75) * i * 5e-7;
        // x += 1. * cos(t * 5.);
        // y += 1. * sin(t * 5.);
        x *= 2.5;
        y *= 2.5;
...
// 
    void main(void) {
        ...
        float osc = map(sin(t * 16e-1), -1., 1., 0.05, 4.005);
        float i = vertexID * 1e-1;
        float x = cos(i * 1e-5 + t * 1e1) * i * 1e-5;
        float y = sin(i * 1e-5 + t * 1e1) * i * 1e-5;
        x *= cos(x / sin(x * 1e2 + x * 1e2) * x * t * 1e2) * 1.65;
        y *= sin(y / sin(y * 1e2 + x * 1e2) * x * t * 1e2) * 1.65;
//         x *= 0.25 * 44.5;
//         y *= 0.25 * 44.5;
